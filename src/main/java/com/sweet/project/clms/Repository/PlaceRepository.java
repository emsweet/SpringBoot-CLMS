...
public interface PlaceRepository extends JpaRepository<Place,Long> {
    @Query(value="SELECT * FROM place WHERE pid IN (" +
            " SELECT pid FROM place AS p WHERE p.type = ?1 AND p.campus = ?2 AND p.building = ?3 AND p.capacity > ?4 \tAND EXISTS ( " +
            "SELECT * FROM reservation AS r WHERE r.place_pid = p.pid AND CONVERT(r.rdate,DATE) = CONVERT(?5 ,DATE) \t" +
            "AND ( CONVERT(r.st_time,TIME) >=CONVERT(?7,TIME) OR CONVERT(r.ed_time,TIME) <= CONVERT(?6,TIME))) )",nativeQuery=true)
